...
.. c:type:: uv_timeval_t
    ...
    Data type for storing times with microsecond resolution.
...
.. c:type:: uv_timespec_t
    ...
    Data type for storing times with nanosecond resolution.
...
.. c:function:: uv_handle_type uv_guess_handle(uv_os_fd_t file)
    ...
    STDIO file descriptor pseudo-handles `UV_STDIN_FD`, `UV_STDOUT_FD`, and
    `UV_STDERR_FD` can be passed to any `uv_os_fd_t`` field for cross-platform
    support of stdio.
...
.. c:function:: int uv_clock_gettime(uv_clock_id clock_id, uv_timespec_t* ts)
