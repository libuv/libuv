...
.. c:function:: int uv_fs_copyfile(uv_loop_t* loop, uv_fs_t* req, const char* path, const char* new_path, int flags, uv_fs_cb cb)
    ...
    - `UV_FS_COPYFILE_FICLONE`: If present, `uv_fs_copyfile()` will attempt to
      ...
      support copy-on-write or an error occurred during trying so, then a
      fallback copy mechanism (:c:func:`uv_fs_sendfile()`) is used.
    - `UV_FS_COPYFILE_FICLONE_FORCE`: If present, `uv_fs_copyfile()` will
      ...
      not support copy-on-write or an error occurred during trying so, then that
      error is returned.
    ...
    .. versionchanged:: 1.33.0 Using `UV_FS_COPYFILE_FICLONE_FORCE` returns
         the actual error instead of `UV_ENOTSUP`.
