...
void on_new_connection(uv_stream_t *server, int status) {
  ...
  if (uv_accept(server, (uv_stream_t *)&new_session->connection) == 0) {
    ...
    // new_session->parser_settings.on_chunk_complete=on_chunk_complete;
    // new_session->parser_settings.on_chunk_header=on_chunk_header;
    ...
    // new_session->parser_settings.on_status=on_status;
